package com.alibaba.datax.plugin.reader.excelreader.utils;
...
import com.alibaba.datax.plugin.reader.excelreader.ExcelReaderErrorCode;
...
public class SftpHelper extends FtpHelper {
	...
	public void loginFtpServer(String host, String username, String password, int port, int timeout,
		...
		try {
			...
			// 如果服务器连接不上，则抛出异常
			if (session == null) {
				throw DataXException.asDataXException(ExcelReaderErrorCode.FAIL_LOGIN,
			...
			if (StringUtils.isNotBlank(password)) {
				...
				String commonPath = StringUtils.join(
						new String[] { System.getProperty("datax.home"), "conf", "common.properties" }, File.separator);
				try {
					FileInputStream fileInputStream = new FileInputStream(new File(commonPath));
					...
					throw DataXException.asDataXException(ExcelReaderErrorCode.FILE_NOT_EXISTS, "您配置的目录文件路径不存在或者没有权限读取");
				}
			...
			if(null != e.getCause()){
				...
				if (unknownHostException.equals(cause)) {
					...
					LOG.error(message);
					throw DataXException.asDataXException(ExcelReaderErrorCode.FAIL_LOGIN, message, e);
				...
				if("Auth fail".equals(e.getMessage())){
					...
					LOG.error(message);
					throw DataXException.asDataXException(ExcelReaderErrorCode.FAIL_LOGIN, message);
				}else{
	...
	public boolean isDirExist(String directoryPath) {
		try {
			...
			LOG.error(message);
			throw DataXException.asDataXException(ExcelReaderErrorCode.COMMAND_FTP_IO_EXCEPTION, message, e);
		}
	}
	...
	public HashSet<String> getListFiles(String directoryPath, int parentLevel, int maxTraversalLevel) {
		if(parentLevel < maxTraversalLevel){
			...
			if (directoryPath.contains("*") || directoryPath.contains("?")) {//*和？的限制
				...
				LOG.error(message);
				throw DataXException.asDataXException(ExcelReaderErrorCode.LINK_FILE, message);
			...
			LOG.error(message);
			throw DataXException.asDataXException(ExcelReaderErrorCode.OUT_MAX_DIRECTORY_LEVEL, message);
		}
	}
