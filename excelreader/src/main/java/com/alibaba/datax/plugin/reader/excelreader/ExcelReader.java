...
import com.alibaba.datax.plugin.reader.excelreader.utils.FtpHelper;
import com.alibaba.datax.plugin.reader.excelreader.utils.SftpHelper;
import com.alibaba.datax.plugin.reader.excelreader.utils.StandardFtpHelper;
...
public class ExcelReader extends Reader {
	public static class Job extends Reader.Job {
		...
		public void init() {
			...
			ExcelParseCheck.validateParameter(this.originConfig);
		...
		private void validateParameter() {
			// todo 常量
			this.protocol = this.originConfig.getNecessaryValue(Key.PROTOCOL, ExcelReaderErrorCode.REQUIRED_VALUE);
			...
			this.host = this.originConfig.getNecessaryValue(Key.HOST, ExcelReaderErrorCode.REQUIRED_VALUE);
			this.username = this.originConfig.getNecessaryValue(Key.USERNAME, ExcelReaderErrorCode.REQUIRED_VALUE);
			...
			this.sheetIndex = originConfig.getString(Key.SHEET, null);

			// only support connect pattern
			this.connectPattern = this.originConfig.getUnnecessaryValue(Key.CONNECTPATTERN,
					Constant.DEFAULT_FTP_CONNECT_PATTERN, null);
			...
			if (!pathInString.startsWith("[") && !pathInString.endsWith("]")) {
				...
				for (String eachPath : path) {
					if (!eachPath.startsWith("/")) {
						...
						LOG.error(message);
						throw DataXException.asDataXException(ExcelReaderErrorCode.ILLEGAL_VALUE, message);
					}
				}
			}
		...
		public void destroy() {
			try {
				...
				String message = String.format("关闭与ftp服务器连接失败: [%s] host=%s, username=%s, port=%s", e.getMessage(),
						host, username, port);
				LOG.error(message, e);
			}
		}
	...
	public static class Task extends Reader.Task {
		...
		public void init() {// 连接重试
			...
			this.sheetIndex = this.readerSliceConfig.getString(Key.SHEET, null);
			...
			if ("sftp".equals(protocol)) {
				...
				this.connectPattern = readerSliceConfig.getString(Key.CONNECTPATTERN,
						Constant.DEFAULT_FTP_CONNECT_PATTERN);// 默认为被动模式
				this.ftpHelper = new StandardFtpHelper();
			}
			ftpHelper.loginFtpServer(host, username, password, port, timeout, connectPattern);
		...
		public void startRead(RecordSender recordSender) {
			...
			for (String fileName : this.sourceFiles) {
				...
				ExcelParserHelper.readFromStream(inputStream, fileName, sheetIndex, this.readerSliceConfig,
